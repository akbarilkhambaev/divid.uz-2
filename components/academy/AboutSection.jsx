...
const defaultSlides = [
  {
    title: 'Darslarda qatnashish',
    subtitle: '(80% amaliyot, 20% nazariy bilimlar)',
    description:
      'Mashg‘ulotlarda yangi mavzuni o‘zlashtirasiz va bilimlarni mustahkamlash uchun uy vazifasini bajarishingiz kerak.',
    videoUrl: '/academy/class-1.webm',
  },
  {
    title: 'Amaliy kurslar',
    subtitle: '(Faqat real biznes keyslar)',
    description:
      'Bizning kurslarimiz faqat nazariy bilimlar bilan chegaralanmaydi.',
  ...
  useEffect(() => {
    const fetchSlides = async () => {
      try {
        ...
        if (!snapshot.empty) {
          const firebaseSlides = snapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
          }));
      ...
      <div className="max-w-7xl w-full">
        ...
        <div className="relative bg-gray-800 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="absolute bottom-8 left-8 flex items-center gap-4 z-10">
            <button
              onClick={prevSlide}
              className="w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm hover:bg-white/20 flex items-center justify-center"
            >
              <svg
