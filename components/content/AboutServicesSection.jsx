...
export default function AboutServicesSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative overflow-hidden w-full bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white pb-20 px-4"
    >
      <ThreeParticlesBg />
      <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-10 w-full about-stat-grid">
        {stats.map((stat) => {
          const IconComponent = ICON_MAP[stat.icon] || FaBriefcase;
          return (
            <div
              key={stat.id}
              className="about-stat-card flex flex-col items-center justify-center rounded-3xl border border-white/10 bg-white/5 p-8 md:p-12 text-center shadow-[0_25px_55px_-20px_rgba(15,23,42,0.8)] backdrop-blur-xl transition-transform duration-300 hover:-translate-y-1"
            >
              <IconComponent className="text-5xl md:text-6xl text-cs-blue mb-4" />
